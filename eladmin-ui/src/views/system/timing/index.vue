  ...
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-if="checkPer(['admin','jobs:edit','jobs:del'])" label="操作" width="150px" align="center">
          <template slot-scope="scope">
            <el-button v-permission="['admin','jobs:edit']" size="mini" style="margin-right: 3px;" type="text" @click="crud.toEdit(scope.row)">编辑</el-button>
            <el-button v-permission="['admin','jobs:edit']" style="margin-left: -2px" type="text" size="mini" @click="execute(scope.row)">执行</el-button>
            <el-button v-permission="['admin','jobs:edit']" style="margin-left: 3px" type="text" size="mini" @click="updateStatus(scope.row)">
              {{ scope.row.triggerState ==='PAUSED' ? '恢复' : '暂停' }}
            </el-button>
            <el-popover
              :ref="scope.row.id"
              v-permission="['admin','jobs:del']"
              placement="top"
              width="200"
            >
              <p>确定停止并删除该任务吗？</p>
...
export default {
  ...
  cruds() {
    return CRUD({ title: 'Quartz集群定时', url: 'api/jobs', idField: 'jobName', optShow: {
      add: true,
      edit: true,
      del: true,
      download: false,
